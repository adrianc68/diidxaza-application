...
import LessonResults from "../../../components/learning/lesson_results/LessonResults";
...
export default function AnswerSection({ lesson }) {
    ...
    useEffect(() => {
        helpHttp()
            .get(UrlAPI + "questions/" + lesson._id, {
                headers: {
            ...
            .then((response) => {
                if (response.length > NUMBER.ZERO) {
                    helpHttp()
                        .get(UrlAPI + "answers/" + response[0]._id, {
                            headers: {
                                Accept: "application/json",
                                "Content-Type": "application/json",
                                Authorization: sessionStorage.getItem("token"),
                        ...
                        .then((responseAnswers) => {
                            if (responseAnswers.length > NUMBER.ZERO) {
                                ...
                                if (responseAnswers.status === RESPONSE_STATUS.INSUFFICIENT_SPACE) {
                                    handleModalAnswer(t("RefreshToken"), () => {
                                        window.location.href = "../login";
                                    });
                                } else {
                                    if (responseAnswers.status === RESPONSE_STATUS.UNAUTHORIZED) {
                                        handleModalAnswer(t("ErrorToken"), () => {
    ...
    const { handleClick, handleClickNext, handleChangeAnswerOnly, handleChangeAnswerMultiple, loading, loadingError, resultsQuestions, pointsObtained } = useLessonForm(
        setQuestion,
        questionsChange,
        setQuestionsChange,
        setAnswers,
    ...
    return (
        (questions.length > NUMBER.ZERO && (
            <form className="answersection-main-container">
                ...
                <div className="result-container">
                    <div className="answersection-answers-container">
                        ...
                        <div className="answersection-answers-unique"></div>
